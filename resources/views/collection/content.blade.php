...
<div class="mt-6 py-[4.5rem]">
  ...
  <section class="mt-6 flex justify-center">
    <div class="w-full max-w-screen-xl grid grid-cols-4 gap-4">
        @forelse ($items as $ci)
    @php
        // 1) Build the link for this item
        switch ($ci->item_type) {
            case 'visual-novel':
                // VNDB uses a “v” prefix
            ...
            default:
                // all other AniList types
                $link = route('media.show', $ci->item_id);
                break;
        }

        // 2) Build the thumbnail URL (you already have this)
        $orig     = $ci->thumbnail_url;
        $thumbUrl = Str::startsWith($orig, 'doujins/')
                  ? Storage::disk('b2')->url($orig)
                  : (Str::startsWith($orig, ['http://','https://'])
                      ? $orig
    ...
    @endphp
        ...
        <div class="relative group">
            {{-- clickable card --}}
        <div class="cursor-pointer card overflow-hidden"
            ...
            <div class="py-3">
                <p class="text-red-600 font-bold">
                    {{ \Illuminate\Support\Str::limit($ci->title, 25) }}
                </p>
                @if($typeLabel)
            ...
            <form method="POST"
                ...
                <button type="submit"
                        ...
                        title="Remove {{ \Illuminate\Support\Str::limit($ci->title, 25) }}">
